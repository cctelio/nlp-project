...
COMMON_EXPORTS+=",ATTN_IMPLEMENTATION=flash_attention_2"
...
COMMON_EXPORTS+=",PACKING=true"
COMMON_EXPORTS+=",PACKING_STRATEGY=bfd_split"
COMMON_EXPORTS+=",PADDING_FREE=false"
...
COMMON_EXPORTS+=",MAX_STEPS=6000"

submit_lr() {
  local lr="$1"
  local job_name="$2"
  local exports="ALL,${COMMON_EXPORTS},LEARNING_RATE=${lr}"

  echo "Submitting ${job_name} with lr=${lr}"
  sbatch --job-name="$job_name" --time=03:00:00 --export="$exports" "$WRAPPER"
}

submit_lr 0.00001 smollm-default-packed-lr1e-5
submit_lr 0.00002 smollm-default-packed-lr2e-5
submit_lr 0.00005 smollm-default-packed-lr5e-5
